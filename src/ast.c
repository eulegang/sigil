...
void sigil_ast_visit_recur(sigil_ast *ast, sigil_slice content, void *ctx,
                           sigil_node node, size_t level,
  ...
  if (node.child != 0) {
    sigil_ast_visit_recur(ast, content, ctx, sigil_ast_nodes(ast)[node.child],
                          level + 1, fn);
  }

  if (node.next != 0) {
    sigil_ast_visit_recur(ast, content, ctx, sigil_ast_nodes(ast)[node.next],
                          level, fn);
  }
}
...
void sigil_ast_visit(sigil_ast *ast, sigil_slice content, void *ctx,
  ...
  sigil_node node = base[cur];

  sigil_ast_visit_recur(ast, content, ctx, node, 0, fn);
}
